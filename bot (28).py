...
# Некоторые хостинги задают токен под другим именем — пробуем несколько вариантов.
_BOT_TOKEN_CANDIDATES = (
    "BOT_TOKEN",
    "TELEGRAM_BOT_TOKEN",
    "TELEGRAM_TOKEN",
    "TOKEN",
...
def _resolve_bot_token():
    for name in _BOT_TOKEN_CANDIDATES:
...
if not BOT_TOKEN:
    available = sorted(
        ...
        if any(s in k.upper() for s in ("TOKEN", "BOT", "TELEGRAM"))
    )
    if available:
        hint = (
            f"\nНайдены похожие переменные: {available}\n"
            "Возможно, токен задан под одним из этих имён — переименуй в BOT_TOKEN "
            "или подправь список в _BOT_TOKEN_CANDIDATES в bot.py."
        )
    else:
        hint = (
            "\nПохоже, переменные окружения не передаются процессу.\n"
            "Проверь, что на хостинге переменная действительно задана для этого сервиса "
            "(вкладка Environment / Variables / Secrets) и что процесс перезапущен."
        )
    raise RuntimeError(
        "Не найден токен Telegram-бота. Установи одну из переменных окружения: "
        + ", ".join(_BOT_TOKEN_CANDIDATES)
        + "."
        + hint
    )
