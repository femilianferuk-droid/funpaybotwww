...
#   "accounts": {golden_key: {"acc": Account, "username": str, "id": int}},
...
def try_add_account(golden_key: str) -> tuple[bool, str, Optional[object]]:
    ...
    try:
        ...
        acc = Account(golden_key, user_agent=DEFAULT_USER_AGENT).get()
        ...
        return (
            ...
            "Проверьте, что ключ скопирован полностью и не истёк.",
            None,
        )
...
def _listener_thread(tg_id: int, bot_token: str, stop_event: threading.Event):
    ...
    acc = data["accounts"][first_key]["acc"]

    # Подменяем user-agent у аккаунта, если он не задан
    if not acc.user_agent:
        acc.user_agent = DEFAULT_USER_AGENT
...
async def cb_add_key(callback: CallbackQuery, state: FSMContext):
    await callback.message.edit_text(
        ...
        "Вы можете найти его в браузере:\n"
        "<i>DevTools → Application → Cookies → funpay.com → golden_key</i>",
...
async def process_golden_key(message: Message, state: FSMContext):
    ...
    await message.answer("⏳ Проверяю ключ, подождите...")

    ok, text, acc = await asyncio.get_event_loop().run_in_executor(
        None, try_add_account, golden_key
    )

    if ok:
        data["accounts"][golden_key] = {
            "acc": acc,
